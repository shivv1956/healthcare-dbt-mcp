...
def create_snowflake_tables(**context):
    ...
    - RAW.FHIR_BUNDLES: Stores FHIR bundle JSON data
    - RAW.LOAD_WATERMARK: Tracks last processed S3 file
    """
    hook = SnowflakeHook(snowflake_conn_id='snowflake_default')
    
    # SQL to create FHIR_BUNDLES table
    create_bundles_table = """
    CREATE TABLE IF NOT EXISTS RAW.FHIR_BUNDLES (
    ...
    CREATE TABLE IF NOT EXISTS RAW.LOAD_WATERMARK (
    ...
    try:
        ...
        logging.info("✓ RAW.FHIR_BUNDLES table ready")
        
        hook.run(create_watermark_table)
        logging.info("✓ RAW.LOAD_WATERMARK table ready")
...
def load_files_to_snowflake(**context):
    ...
    create_stage_sql = f"""
    CREATE OR REPLACE STAGE RAW.S3_FHIR_STAGE
    ...
    try:
        ...
        count_query = "SELECT COUNT(*) FROM RAW.FHIR_BUNDLES;"
...
def update_watermark(**context):
    ...
    insert_watermark = f"""
    INSERT INTO RAW.LOAD_WATERMARK (LAST_PROCESSED_KEY, LAST_PROCESSED_TIME, FILES_PROCESSED)
